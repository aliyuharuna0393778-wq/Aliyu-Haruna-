...
# ---- Database connection ----
# Set DATABASE_URL as an environment variable on Render (Environment tab).
# Get this connection string from your free Neon.tech project dashboard.
...
# Tables included in backup/restore, in an order safe for restoring
BACKUP_TABLES = [
    "students", "admins", "announcements", "lessons",
    "results", "assignments", "assignment_submissions", "login_log"
]
...
class DBWrapper:
    ...
    sqlite3-style calls like conn.execute(query, params).fetchone()."""
...
def admin_backup():
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=students_backup.json"}
    )
...
def admin_restore():
    ...
    if request.method == "POST":
        ...
        if not file.filename.endswith(".json"):
            flash("That doesn't look like a valid backup file (.json).", "error")
            return redirect(url_for("admin_restore"))

        try:
            data = json.loads(file.read().decode("utf-8"))
        except Exception:
            flash("Could not read that backup file.", "error")
            return redirect(url_for("admin_restore"))
        ...
        try:
            ...
            for table in BACKUP_TABLES:
                ...
                # Make sure future new rows don't collide with restored IDs
                conn.execute(
